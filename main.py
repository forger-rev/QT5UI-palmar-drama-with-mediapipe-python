...
def detection(image,lh_l, fa_l, po_l, dp, self):
    ...
    # print(body_language_class, str(round(body_language_prob[np.argmax(body_language_prob)],3)))
    gclass = 0

    if body_language_class[0] == "S" :
        gclass = 0
    if body_language_class[0] == "M" :
        gclass = 1
    if body_language_class[0] == "Y" :
        gclass = 2
    if body_language_class[0] == "T" :
        gclass = 3
    if body_language_class[0] == "H" :
        gclass = 4
    if body_language_class[0] == "R" :
        gclass = 5

    # Extract landmarks
    landmarks = po_l.landmark
    landmarks_d = dp.landmark
    
    # Get coordinates
    L_shoulder = [landmarks[mp.solutions.holistic.PoseLandmark.LEFT_SHOULDER.value].x,landmarks[mp.solutions.holistic.PoseLandmark.LEFT_SHOULDER.value].z]
    R_shoulder = [landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_SHOULDER.value].x,landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_SHOULDER.value].z]
    R_wrist = [landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_WRIST.value].x,landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_WRIST.value].z]
    
    show_point = [landmarks_d[mp.solutions.holistic.PoseLandmark.RIGHT_SHOULDER.value].x,landmarks_d[mp.solutions.holistic.PoseLandmark.RIGHT_SHOULDER.value].y]
    
    # Calculate angle
    angle_s = calculate_angle(L_shoulder, R_shoulder, R_wrist)

    # Visualize angle
    cv2.putText(image, str(angle_s), 
                    tuple(np.multiply(show_point, [image.shape[1], image.shape[0]]).astype(int)), 
    ...
    coords = tuple(np.multiply(
                    np.array(
                        (dp.landmark[mp.solutions.holistic.PoseLandmark.LEFT_EAR].x, 
                            dp.landmark[mp.solutions.holistic.PoseLandmark.LEFT_EAR].y))
    ...
    # Grab R_index_f coords
    coords_ri = tuple(np.multiply(
                    np.array(
                        (dp.landmark[mp.solutions.holistic.PoseLandmark.RIGHT_INDEX].x, 
                            dp.landmark[mp.solutions.holistic.PoseLandmark.RIGHT_INDEX].y))
                , [image.shape[1], image.shape[0]]).astype(int))

    # Grab L_index_f coords
    coords_li = tuple(np.multiply(
                    np.array(
                        (dp.landmark[mp.solutions.holistic.PoseLandmark.LEFT_INDEX].x, 
                            dp.landmark[mp.solutions.holistic.PoseLandmark.LEFT_INDEX].y))
                , [image.shape[1], image.shape[0]]).astype(int))


    if gclass == 2 or gclass == 3 :
        ...
        (landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_WRIST.value].y)):
            ...
            cv2.rectangle(image, 
                    (coords_ri[0], coords_ri[1]+5), 
                    (coords_ri[0]+len('Too Low')*20, coords_ri[1]-30), 
                    (255, 0, 0), -1)
            cv2.putText(image, 'Too Low', coords_ri, 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

        if ((landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_SHOULDER.value].y-0.03)>
        (landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_WRIST.value].y)):

            cv2.rectangle(image, 
                    (coords_ri[0], coords_ri[1]+5), 
                    (coords_ri[0]+len('Too High')*20, coords_ri[1]-30), 
                    (255, 0, 0), -1)
            cv2.putText(image, 'Too High', coords_ri, 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2, cv2.LINE_AA)   

        accu = float(10-height_s*100+0.15)
        # print(accu)
    ...
    if gclass == 0 :
        ...
        accu = float(10-height_h*100+0.15)

    if gclass == 1 :
        height_h_1 = (landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_SHOULDER.value].y)
        +0.033*np.abs((landmarks[mp.solutions.holistic.PoseLandmark.LEFT_SHOULDER.value].x)-
        (landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_SHOULDER.value].x))
        height_h_2 = landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_WRIST.value].y

        height_h = (height_h_1-height_h_2)**2

        if (height_h_1+0.12)<(height_h_2):
    ...
    if gclass == 4 :
        height_h = ((landmarks[mp.solutions.holistic.PoseLandmark.LEFT_SHOULDER.value].y)-
        (landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_WRIST.value].y))**2

        if ((landmarks[mp.solutions.holistic.PoseLandmark.LEFT_SHOULDER.value].y+0.12)<
        (landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_WRIST.value].y)):
        ...
        if ((landmarks[mp.solutions.holistic.PoseLandmark.LEFT_SHOULDER.value].y-0.03)>
        (landmarks[mp.solutions.holistic.PoseLandmark.RIGHT_WRIST.value].y)):
    ...
    if gclass == 5 :
        height_h = ((landmarks[mp.solutions.holistic.PoseLandmark.LEFT_SHOULDER.value].y)-
        (landmarks[mp.solutions.holistic.PoseLandmark.LEFT_WRIST.value].y))**2

        if ((landmarks[mp.solutions.holistic.PoseLandmark.LEFT_SHOULDER.value].y+0.12)<
        (landmarks[mp.solutions.holistic.PoseLandmark.LEFT_WRIST.value].y)):

            cv2.rectangle(image, 
                    (coords_li[0], coords_li[1]+5), 
                    (coords_li[0]+len('Too Low')*20, coords_li[1]-30), 
                    (255, 0, 0), -1)
            cv2.putText(image, 'Too Low', coords_li, 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
        ...
        (landmarks[mp.solutions.holistic.PoseLandmark.LEFT_WRIST.value].y)):
            ...
            cv2.rectangle(image, 
                    (coords_li[0], coords_li[1]+5), 
                    (coords_li[0]+len('Too High')*20, coords_li[1]-30), 
                    (255, 0, 0), -1)
            cv2.putText(image, 'Too High', coords_li, 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2, cv2.LINE_AA)
...
def angle_function(image,po_l,dp) :
    # Extract landmarks
    landmarks = po_l.landmark
    landmarks_d = dp.landmark
    ...
    show_point = [landmarks_d[mp.solutions.holistic.PoseLandmark.RIGHT_SHOULDER.value].x,landmarks_d[mp.solutions.holistic.PoseLandmark.RIGHT_SHOULDER.value].y]
    # Calculate angle
    angle = calculate_angle(L_shoulder, R_shoulder, R_wrist)
    ...
    # # Render detections
    # mp.solutions.drawing_utils.draw_landmarks(image, po_l, mp.solutions.pose.POSE_CONNECTIONS,
...
def calculate_angle(a,b,c):
    ...
    angle = np.abs(radians*180.0/np.pi)*100//1/100
...
def plot_world_landmarks(
    ...
    if count == 0:
        ...
        label_show(label=self.labelVideoSample, image=image_3d)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def initUI(self):
        ...
        self.pushButton_3.clicked.connect(self.load_video_T3)
        self.pushButton_4.clicked.connect(self.load_video_T4)
    ...
    def load_video(self):
        ...
        while True:
            ...
            #     label_show(label=self.labelVideoSample, image=img_3d) 
            #     seconds = time.time()
            #     de_sec = seconds*10
            #     count = (de_sec//1)%10
            #     if count == 0:             
    ...
    def start_detect(self): 
        ...
        #         label_show(label=self.labelImageOrVideo, image=rectangle)
        #         if success:
        #             label_show(label=self.labelVideoSample, image=hand)
        #         else:
        #             label_show(label=self.labelVideoSample, image=maya)
        #         cv2.waitKey(int(1000 / self.videoFPS))
        # if self.camera:
    ...
    def predict(self):
        ...
        while self.mode_key == 0 :
            ...
            image_width, image_height = image.shape[1], image.shape[0]
            cv2.rectangle(image, (0, 0), (180, 480), (204, 255, 102), -1) # cover for user

            n=0
            while True:
                cv2.line(debug_image, (10, n), (20, n), (204, 255, 102), 2) 
                cv2.putText(debug_image, str(n), (30, n),
            ...
            n=0
            while True:
                cv2.line(debug_image, (n, 10), (n, 20), (204, 255, 102), 2) 
                cv2.putText(debug_image, str(n), (n, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1,
                        cv2.LINE_AA)
                n=n+50
                if n>=image_width:
            ...
            gclass = 0
            # **run draw function after detection
            ...
            if (face_landmarks is None) or (pose_landmarks is None) or (right_hand_landmarks is None):
                ...
                if (right_hand_landmarks is not None) and (face_landmarks is not None) and (pose_landmarks is not None):
                    (debug_image, gclass) = detection(debug_image, 
                    right_hand_landmarks, face_landmarks, t_pose_landmarks, pose_landmarks ,self)
                    # image has been flipped <swap R and L>
            ...
            if face_landmarks is not None:
                ...
                # Check the Height of Left Hand
                if ((coords_re > coords_le + 10) or (coords_re < coords_le - 10)) and gclass != 4 :
                    draw_c = (0,0,255)
